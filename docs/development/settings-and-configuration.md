...
- reset/backup 是否应保留该配置；当前 reset 保留 allowlisted 配置与 credential reference（含 v44–v48 的上下文模式），受支持旧版来源与默认值见[存储生命周期](../architecture/storage-and-data-lifecycle.md#schema-发布策略)，不复制或恢复操作系统 secret。通知事件、Browser history/download records 或 Agent template library 不保留；
