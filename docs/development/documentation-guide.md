...
开发文档对齐当前仓库实现，不等同于某个已发布安装包的能力清单。核验包含未提交改动时，应在相关页面
说明工作区基线；区分“已持久化/可编辑”“已接入执行”和“计划支持”，尤其不能从 UI 入口、类型或表结构
推断执行链路已经完成。发布版本、下载说明和用户升级承诺应在对应发布流程中单独核验。
...
定期维护时先比较上次核验以来的提交与当前工作区改动，再按模块追踪代码、协议、测试和相邻文档。
不要批量刷新所有页面的 `last_verified`；仅更新实际核验过的页面。存储迁移与恢复工具的支持范围应分别
检查，不能因为 schema 常量已升级，就推断历史数据恢复路径也已升级。
...
检查器通过只证明其覆盖的结构、链接和漂移规则通过，不代表所有行为描述都与实现一致。收口时仍需
交叉核验状态所有权、失败/恢复分支和“尚未实现”的边界，并如实区分“阅读测试确认契约”与“实际运行测试”。
