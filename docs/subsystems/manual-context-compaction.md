...
用户在空白输入框中键入 `/` 打开本地命令面板。首批命令为压缩上下文、新聊天、使用情况和计费、置顶/取消置顶、重命名、归档；创建聊天分支不在此面板中。新聊天、元信息操作和用量设置复用现有应用入口。

## 输入与展示

面板按中文名称或稳定英文别名 `compact`、`new`、`usage`、`pin`、`rename`、`archive` 实时筛选，突出匹配文字。鼠标悬停与方向键共享选中行，Enter 执行该行；Escape 收起面板并保留文字，Tab 按正常焦点顺序导航。已有附件不妨碍通过空白文本打开面板。
...
手动压缩复用 Provider transition 的时间线分割线样式与运行文字效果，开始显示“正在压缩上下文”，成功显示“上下文历史已压缩”。分割线右侧不显示取消按钮；后端取消接口仍保留。时间线锚点只用于展示，不是计费 owner 或模型游标；原始消息不变。操作按 conversation/operation identity 合并，迟到的 running 通知不能覆盖终态；切换聊天和重新加载从 Host 查询持久状态。完成后刷新上下文占用。
...
SQLite v35 新增操作表和独立用量表，仅对 exact v34 schema 提供事务升级，原始历史与费用完整保留。未知版本和校验不通过的库仍 fail closed。详见[存储生命周期](../architecture/storage-and-data-lifecycle.md)。
...
Fork contract 增加 `{ kind: 'latest' }`，由 Host 解析当前有效模型、最新摘要和完整历史边界。最新位置分支复制上下文和展示来源，不复制费用；历史回复分支保持自己的历史边界，不取得其后的手动摘要。此阶段不新增 Slash 分支入口。
