...
基础提示词保留跨工具的信任、权限、审批和沟通边界；工具总说明保留选择工具与首次调用所需的关键提醒，字段说明集中解释参数条件。精简重复文字不改变工具集合、参数分支、限额或执行权限。无工作区的首次命令目录、Skill 的 Run 作用域与窄工作流例外、文件凭据复用、事务未结算时的限制以及命令未知终态等历史修复语义必须保留。创建成功返回的准确 `fileChangeTarget` 可以供后续模型响应复用；未取得可复用凭据时，更新和删除仍须先读取准确目标。Skill 的模型目录使用 `ref` 字段，激活时原样传入 `skills_activate.skillRef`，不能把固定 Skill ID 当作调用参数。

说明文字也参与稳定 ToolSet 的版本指纹。发布文案修改前应先完成或取消旧的审批、交互暂停及其他未结束 Run，再重启加载新版本；不能忽略指纹或替换旧 Checkpoint 的定义来强行恢复。部分冻结动作可能已执行、随后在模型续跑时才发现版本不匹配，因此恢复失败不能推断动作未发生。基础上下文预算测试核对真实请求与预览的固定前缀统计，数字使用本地估算口径，不代表供应商实际 Token；文字及 Schema 测试不能替代真实模型的行为回归。
