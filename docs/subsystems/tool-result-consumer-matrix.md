...
本文记录 canonical Tool Call/Tool Result 到各消费者的职责映射。它是评审指南，不是运行时 allowlist；机器可校验的字段契约以 `ToolRegistry` 投影函数和 `crates/core/tests/fixtures/tool_result_projection_contract_v1.json` 为准。权限与审批见[Tool 体系、权限与审批](./tools-permissions-and-approvals.md)，FileChange 的执行/恢复 authority 见[FileChange 子系统](./file-change.md)，大小边界见[Tool Result 上限、分页与恢复](./tool-result-limits.md)，后台报告的业务消费者见[Scheduled Automation 子系统](./scheduled-automations.md)。
...
| `apply_patch`                                                    | Direct/Staged receipt；成功写后仅 M 获得 successor Observation      | E/R 显示 transient preview、分页 Diff、draft/progress 与审批状态 | T/A 的 call body-free、结果无 successor；私有 C/audit 保存恢复 binding  | 唯一专用结构化文本修改 Tool；公共 Diff/Observation 都不是执行 authority                      |
...
| `office_document` / `office_spreadsheet` / `office_presentation` | documentKind/operation、outputs、进程终态与不确定性                 | E 展示 Office 活动和产物                                         | A 保存 stdout/stderr；T/C 保存 execution/prepared identity 与 outputs   | 模型面仅检查/验证/渲染；render 走 `OfficeOperation` 审批                                     |
...
| Skill script preflight/run                                       | runtime/requirements 摘要、argv 结果、stdout/stderr                 | E 展示执行活动                                                   | Run 的输出进入 A；T/C 保存 frozen script/digest                         | 两者均要求 read/write all + Full Access；仅已核验应用内置来源可按 builtin execution 自动审批 |
...
所选 Folder Reference 不属于受管 Artifact 存储路径：模型输入有意提供用户选择的 name/绝对 `rootPath`，基于它读取的 Tool 可以返回相同可读路径；Host 的目录 identity、内部状态和导入暂存路径仍私有。不能把“受管路径不得泄漏”扩大成删除所有合法外部展示路径，也不能仅凭展示路径获得 authority。详见[会话输入](conversation-inputs.md#文件夹引用)。

FileChange 进一步收紧这些边界：

- `apply_patch` durable call projection 只保存 action/operation/path/transaction/revision、byte/edit count 与 digest，不保存完整 content、old/new text 或 `observationId`。
- 成功 create/update/delete/commit 的 `fileChangeTarget` 只进入 M 和私有 checkpoint；E/T/A 统一剥离 `observationId`、`fileChangeTarget`、`observationRefreshRequired` 与 continuation。
- Direct/Staged commit 的精确 binding 与 terminal receipt 由 Host 私有 pending action/action audit 保存；Staged 草稿正文另由 FileChange store 保存。它们是审批恢复和历史 Diff authority，不是公共 Tool Result 或 Exact Archive 的授权用途。
- `agent.getFileChangeHistoryDiff` 从 terminal audit 懒加载：Direct 使用冻结 inline Diff，Staged 从 binding 重建；Trace 中的 body-free call 不能用于重建正文。
...
- Managed Playwright screenshot 可同时返回经验证的 `image-artifact://` readPath；下载返回 Rust Core 持久化、无宿主路径的 `browser-download:<uuid>` 引用，并在后续文件输入时重新校验 Conversation/Agent tree/Project scope。外部 MCP Server 的原始 resource/binary block 不穿过 Core Server/Main 安全投影边界。
...
8. FileChange successor Observation 不得进入 E/T/A；历史 Diff 只从授权的 terminal audit/transaction 读取。
...
- FileChange 投影：`crates/core/src/tools/apply_patch.rs`、`crates/core/src/file_change_support.rs`、`crates/core/src/runtime/checkpoint.rs`
...
- [ ] FileChange call body、Observation authority 与 successor 只进入允许的私有投影，历史 Diff 从 durable audit 懒加载。
