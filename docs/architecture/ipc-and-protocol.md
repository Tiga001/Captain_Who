...
这些都是 transport/domain envelope 版本，不是 SQLite schema。当前 canonical SQLite schema 是 v47，由 Rust Core storage 独立校验；Renderer、Preload 和 Main 不读取、协商或转发数据库 schema version。
...
10. Automation DTO schema v1、permission mode v2 和 SQLite schema v47 必须分别命名、分别验证。
