...
| 极简/完整模式 `interaction.profile.contextProfile` | Conversation；记录当前 Run 冻结的 `full` / `minimal` 及简短说明，独立于工作模式和扩展开关 |
...
极简模式与完整模式通过同一请求准备入口选择基础提示词、工具定义和 World State，再生成完整请求计量；模式及最终定义参与配置指纹。设置保存后，空闲预览采用下一轮模式，正在运行及等待审批的 Run 保持冻结模式，不能把下一轮预估覆盖本轮快照。自动压缩阈值仍使用这份相同的计量结果，不另设“极简模式 Token 算法”。
