...
Automation DTO/notification envelope 使用 schema v1；它与 SQLite canonical schema v24 是两条独立版本线。Main 不解析 SQLite schema，也不把数据库版本暴露给 Renderer。
...
10. Automation DTO schema v1 与 SQLite schema v24 不得由 Main 合并为单一“自动化版本”。
