...
Notification DTO 使用 schema v1，Automation DTO 使用独立 schema v1/permission mode v2；它们与 SQLite canonical schema v27 是三条独立版本线。Main 不解析 SQLite schema，也不把数据库版本暴露给 Renderer。
...
10. Notification schema v1、Automation schema v1/permission mode v2 与 SQLite schema v27 不得由 Main 合并为单一版本。
