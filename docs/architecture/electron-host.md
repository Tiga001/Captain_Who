...
| 运行时                   | 当前职责                                                                                                                                                                              | 不应承担的职责                                                               |
| ------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------- |
| Electron Main            | 冻结应用数据目录；创建窗口；启动和关闭 Core Server；注册受信 IPC；持有原生对话框、原生通知、文件路径、WebContents、Session、浏览器 Target 和 CDP 权限；协调通用通知、终端与浏览器服务 | 不渲染产品 UI；不把原生对象或绝对托管路径交给 Renderer                       |
...
Electron Main
    ...
    ├── durable notification batch lease ── Electron Notification
...
2. 注册自定义资源 scheme，构造 Core Server 门面、通知 locale store 和终端桥，但尚不创建窗口；favicon 资源缓存、Browser Broker、网络 Guard 和 Managed Playwright Host 要到 `app.whenReady()` 后才创建。
...
7. 注册 Host API IPC；所有 handler 都通过受信发送方包装器。Notification registrar 创建 Main-owned 原生通知 coordinator，并订阅通用 notification event/resync；Automation registrar 只负责 Scheduled 领域事件与业务请求。
8. 若窗口已可见则开始原生通知 delivery，最后标记 Host/startup readiness ready。窗口若稍后才显示，则在 activate/ready-to-show 后开始 delivery。
...
原生通知交付由 Main 拥有；普通 `human_root` 任务与 Automation 的通知事实、合并批次和投递状态由 Core Server/Rust Core 的 SQLite 持有。事实、设置、批次、点击和失败恢复的完整契约见[通用通知](../subsystems/notifications.md)；Automation task/Run 的业务状态仍见 [Scheduled Automation](../subsystems/scheduled-automations.md)。完整链路为：
...
`notification.event` 和 `notification.resync` 只唤醒 coordinator、降低轮询延迟或提示通知设置重载；它们不是交付真相。Host 初始化期间 coordinator 保持暂停，首个应用窗口可见后立即 drain，随后每 30 秒主动 drain；当前单次最多领取 10 个批次、claim lease 60 秒、显示超时 15 秒。

交付必须保持以下顺序：

1. 平台不支持 Electron Notification 时把批次持久结算为 suppressed；所属 Conversation、审批或 Scheduled 状态仍留在 SQLite。
...
Notification DTO 使用 schema v1，Automation DTO 使用独立 schema v1/permission mode v2；它们与 SQLite canonical schema v32 是三条独立版本线。Main 不解析 SQLite schema，也不把数据库版本暴露给 Renderer。
...
1. 标记正在退出，立即停止通用原生通知 producer，并让窗口生命周期控制器进入退出模式。停止 producer 必须发生在第一个 `await` 前，避免通知 drain 惰性重启 Core Server。
...
10. Notification schema v1、Automation schema v1/permission mode v2 与 SQLite schema v32 不得由 Main 合并为单一版本。
...
- 通知 Preload bridge：`src/preload/NotificationIpcBridge.ts`
- Browser surface/内部页：`src/main/browser/BrowserSurfaceManager.ts`、`BrowserInternalPageStore.ts`
- Browser 历史与链接：`src/main/browser/BrowserHistoryService.ts`、`BrowserLinkRouter.ts`
- Browser 下载：`src/main/browser/BrowserDownloadBroker.ts`、`src/main/ipc/browserDownloadIpc.ts`
...
- [ ] 通用 notification producer 在 shutdown 的第一个异步等待前停止，Host-only claim/ACK 不会由 Renderer 触发。
