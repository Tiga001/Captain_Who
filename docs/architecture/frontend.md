...
Automation 共享 DTO 使用 `AUTOMATION_SCHEMA_VERSION = 1`，permission mode 使用独立的 v2；它们不是 SQLite canonical schema。当前 SQLite schema 是 v34，Renderer 不读取或协商该数据库版本。
...
10. Automation DTO schema v1、permission mode v2 与 SQLite schema v34 必须分开命名和演进。
