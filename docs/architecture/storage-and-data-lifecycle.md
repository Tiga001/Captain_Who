...
截至本次核验，当前唯一受支持的 canonical schema 是 **v16**（SQLite `PRAGMA user_version = 16`）：

- `STORAGE_SCHEMA_VERSION = 16`；
- canonical schema fingerprint 为 `sha256:78a36d4183ab5a93699faaa300fff3c3016bda199974f50cc0ebd094173eefd0`，由编译期常量和测试固定；
...
Bootstrap 大致执行：解析数据根与锁、打开/校验 canonical schema v16、构造 repositories/services、加载 MCP Server/Provider/Skills/Artifact Runtime、随后运行领域 reconciliation。
...
1. `canonical_schema.sql`、canonical schema v16 的 version 与 fingerprint 必须一致。
...
- [ ] 修改 `canonical_schema.sql` 后同步 canonical version、fingerprint 和 fresh-schema 测试；若版本不再是 v16，同时更新本文当前快照。
