...
截至本次核验，当前唯一受支持的 canonical schema 是 **v23**（SQLite `PRAGMA user_version = 23`）：

- `STORAGE_SCHEMA_VERSION = 23`；
- canonical schema fingerprint 为 `sha256:3e93868dfdae770b7c3d152b4112b48b44dbfc744e0cf85da0ac90366f0657a9`，由编译期常量和测试固定；
...
Automation 在 canonical schema v23 中使用四张表，完整列、CHECK、索引和 trigger 仍以 DDL 为准：
...
Bootstrap 大致执行：解析数据根与锁、打开/校验 canonical schema v23、构造 repositories/services、加载 MCP Server/Provider/Skills/Artifact Runtime、随后运行领域 reconciliation。
...
1. `canonical_schema.sql`、canonical schema v23 的 version 与 fingerprint 必须一致。
...
- [ ] 修改 `canonical_schema.sql` 后同步 canonical version、fingerprint 和 fresh-schema 测试；若版本不再是 v23，同时更新本文当前快照。
