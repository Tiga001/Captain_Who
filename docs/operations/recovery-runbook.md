...
当前基线为 **schema v35 + exact catalog fingerprint + valid foreign keys**；经过完整校验的 exact v34 可自动进行事务升级，保留聊天、trace、摘要、配置、凭据引用与用量。真源：
...
除已支持的 exact v34 外，旧版本、非空未版本化库、catalog 漂移或外键违规均返回 `development_storage_schema_reset_required`。v34 升级在一个事务中执行并复核 v35 catalog；失败回滚，不自动 reset。升级前可保留私有副本，升级后的 v35 不支持直接交给旧应用打开。
...
2. 从 exact current v35 schema 提取 allowlisted configuration；现有受限选项也支持绑定 exact v33 fingerprint 的私有备份设置恢复。MCP 精确 identity/authorization 必须重新验证，未知 schema 仍拒绝。正常 v34 保数据升级不应调用 reset。
3. 在 staging 文件创建 fresh v35 canonical DB。
...
- 不要直接把一个旧 schema backup 覆盖回运行路径并期待 v35 接受；旧库仍会触发 reset-required。
- 如必须人工还原文件，先停止所有 Core Server、再次复制保存当前文件、在隔离位置验证 SQLite 完整性和 schema，再决定是否替换。仓库当前没有受支持的一键 backup restore 命令。
- 当前 v35 SQLite backup 不含当前模型/搜索 secret，只含 reference 与非秘密元数据；旧 schema backup 仍可能含明文 Token/Key，二者都不得上传到 issue、CI artifact 或公共对象存储。
...
当本开发分支必须把旧 schema 的真实开发库改成 v35，不能把兼容读取长期留在产品中。维护者应：
...
3. 只在代码审阅过的临时转换块明确绑定该 exact 旧 schema/fingerprint、固定字段清单和目标 v35 时，运行 `pnpm storage:reset-dev -- --confirm-reset`。转换块不得接受未知 schema、模糊列或任意历史版本。
4. 让工具先生成权限受限、通过 SQLite 校验的备份，再把 allowlisted 设置写入 fresh staging v35；Provider secret 写入当前 Credential Store，SQLite 只落 reference。任一步失败都保留原库和备份，不发布半成品。
5. 验证 `PRAGMA user_version = 35`、catalog fingerprint、`quick_check`、`foreign_key_check`，再启动应用逐项核对模型/搜索/图片凭据状态、UI/Prompt 偏好、Skill、MCP、通知和 Browser 设置；不得用真实付费请求作为唯一验证。
...
- 备份没有自动 retention、加密或异地复制；当前 v35 SQLite snapshot 不含当前模型/搜索 secret，但旧 schema backup 或未签名 macOS 开发环境的整根备份可能含明文凭据。
