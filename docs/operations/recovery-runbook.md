...
当前基线为 **schema v48 + exact catalog fingerprint + valid foreign keys**；exact v47 支持保留历史的 v48 搜索索引事务升级；其他旧版仍须显式 reset，不迁移旧聊天、运行和检查点。真源：
...
旧版本（含 v34–v46）、非空未版本化库、catalog 漂移或外键违规均返回 `development_storage_schema_reset_required`，不改写原库，不自动 reset。v48 不支持直接交给旧应用打开。
...
2. 从 exact current v48 及 exact v35–v47 schema 提取 allowlisted configuration；现有受限选项也支持绑定 exact v33 fingerprint 的私有备份设置恢复。MCP 精确 identity/authorization 必须重新验证；未知配置结构拒绝重置，不能用默认值默默替换模型配置。
3. 在 staging 文件创建 fresh v48 canonical DB。
...
- 通知设置；来自 exact v36–v48 时还保留人机交互 enabled/revision/updatedAt，v43–v48 还保留全局协作开关及 revision，v44–v48 保留上下文模式（轻量/完整），更早版本的上下文模式默认按 Full 处理；
...
- 当前 v48 SQLite backup 不含当前模型/搜索 secret，只含 reference 与非秘密元数据；旧 schema backup 仍可能含明文 Token/Key，二者都不得上传到 issue、CI artifact 或公共对象存储。
...
exact v35–v47 使用上述受管 reset 流程提取白名单配置并新建 v48；既有 exact v33 私有备份恢复仍绑定固定 fingerprint。未支持的旧结构若包含任何配置表，工具拒绝重置并保留原库。不要修改版本号骗过检查，也不要临时写聊天、运行或检查点迁移绕过开发期数据策略。

重建后验证 `PRAGMA user_version = 48`、catalog fingerprint、`quick_check`、`foreign_key_check`，再核对模型、搜索和图片凭据状态，以及 UI/Prompt、Skill、MCP、通知、Browser 和人机交互设置。无需真实付费请求来验证配置保留；历史备份继续按敏感材料保管。
...
- 备份没有自动 retention、加密或异地复制；当前 v48 SQLite snapshot 不含当前模型/搜索 secret，但旧 schema backup 或未签名 macOS 开发环境的整根备份可能含明文凭据。
