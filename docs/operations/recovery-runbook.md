...
当前基线为 **schema v49 + exact catalog fingerprint + valid foreign keys**；exact v47 支持依次保留历史地升级到 v48、v49（搜索索引与纯附件引导）；其他旧版仍须显式 reset，不迁移旧聊天、运行和检查点。真源：
...
旧版本（含 v34–v48）、非空未版本化库、catalog 漂移或外键违规均返回 `development_storage_schema_reset_required`，不改写原库，不自动 reset。v49 不支持直接交给旧应用打开。
...
2. 从 exact current v49 及 exact v35–v48 schema 提取 allowlisted configuration；现有受限选项也支持绑定 exact v33 fingerprint 的私有备份设置恢复。MCP 精确 identity/authorization 必须重新验证；未知配置结构拒绝重置，不能用默认值默默替换模型配置。
3. 在 staging 文件创建 fresh v49 canonical DB。
...
- 通知设置；来自 exact v36–v49 时还保留人机交互 enabled/revision/updatedAt，v43–v49 还保留全局协作开关及 revision，v44–v49 保留上下文模式（轻量/完整），更早版本的上下文模式默认按 Full 处理；
...
- 当前 v49 SQLite backup 不含当前模型/搜索 secret，只含 reference 与非秘密元数据；旧 schema backup 仍可能含明文 Token/Key，二者都不得上传到 issue、CI artifact 或公共对象存储。
...
exact v35–v48 使用上述受管 reset 流程提取白名单配置并新建 v49；既有 exact v33 私有备份恢复仍绑定固定 fingerprint。未支持的旧结构若包含任何配置表，工具拒绝重置并保留原库。不要修改版本号骗过检查，也不要临时写聊天、运行或检查点迁移绕过开发期数据策略。
...
- 备份没有自动 retention、加密或异地复制；当前 v49 SQLite snapshot 不含当前模型/搜索 secret，但旧 schema backup 或未签名 macOS 开发环境的整根备份可能含明文凭据。
