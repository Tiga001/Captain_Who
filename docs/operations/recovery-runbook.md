...
当前唯一可接受基线为 **schema v19 + exact catalog fingerprint + valid foreign keys**。真源：
...
3. 在 staging 文件创建 fresh v19 canonical DB。
...
- 不要直接把一个旧 schema backup 覆盖回运行路径并期待 v19 接受；旧库仍会触发 reset-required。
