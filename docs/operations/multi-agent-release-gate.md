...
脚本的 storage step 当前明确标为 “canonical v20”。实际版本的唯一真源仍是 `crates/core/src/storage/migrations.rs`；修改 schema 时必须同步 runner label 与本门禁，不能仅凭日志文字判断兼容性。
...
当前 canonical storage 为 **v20**，且只有 normalized SQLite catalog fingerprint 完全匹配才接受。以下输入必须 fail closed 且不修改源库：
...
稳定错误标识为 `development_storage_schema_reset_required`。开发 reset 必须先 dry-run、取得 exact DB lock、创建并验证私有备份、构造 fresh v20、恢复 allowlisted 配置、执行 `quick_check`/`foreign_key_check` 后原子发布。详见 [恢复 Runbook](recovery-runbook.md)。
