...
| 动态 Runtime Extension                  | 原样接收并归档 Provider Tool Result                              | Tool 投影后统一经过 10K Gate                                                    | Provider `cursor`/`next`/`continueWith` 或 Archive `historyOpen` | M/E/R/T/A/C | Provider 声明的安全限 + 中央 10K                                                                 |
| MCP Tool                                | transport 4 MiB 硬限后生成安全投影；原始结果不进入 Exact Archive | text 16 KiB、structured JSON 8 KiB，再经中央 Gate；binary/resource 只给省略诊断 | 重新调用必须重新审批；`OutcomeUnknown` 禁止自动 retry/replay     | M/E/T/C     | 128 blocks；单 binary 1 MiB、总 binary 2 MiB；structured depth 32/nodes 4,096                    |
...
- 文档和网页 Provider 解析阶段仍可能需要在内存中形成文本；进程输出已经使用流式
  spool。
- 当前 MCP 有独立的 stdio transport、Manager 和 Catalog，并通过统一 ToolRegistry、
  approval、trace 与模型投影主链路执行。MCP 原始参数和原始结果不进入 Exact Archive；
  只有安全、有界的模型/Event/Trace/Checkpoint 投影可以离开 Host 边界。新增 transport
  必须复用这些限制，不能建立旁路。
