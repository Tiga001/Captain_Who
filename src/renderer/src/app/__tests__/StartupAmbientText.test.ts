...
describe('pickNextStartupPhraseIndex', () => {
  it('keeps a localizable startup shell before React mounts', () => {
    ...
    expect(rendererShell).toContain('background: rgba(16, 16, 16, 0.32)')
    expect(rendererShell).toContain('background: rgba(244, 244, 242, 0.16)')
    expect(rendererShell).toContain('color: #ffffff')
    expect(rendererShell).toContain('color: #000000')
    expect(rendererShell).toContain('width: 48px')
    expect(rendererShell).toContain('height: 38px')
    expect(rendererShell).toContain("html[data-color-scheme='dark'] .app-bootstrap-screen")
  ...
  it('resolves the existing brand artwork through the Renderer asset pipeline', () => {
    ...
    expect(bootstrapEntry).toContain('getBootstrapStartupAppearance')
    expect(bootstrapEntry).toContain('applyBootstrapStartupAppearance')
    expect(bootstrapEntry).toContain('darkBrandMark : lightBrandMark')
  })
