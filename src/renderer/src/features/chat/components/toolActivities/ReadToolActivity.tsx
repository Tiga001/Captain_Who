...
function getPathFromCall(call: AgentToolCall) {
  if (!call.args || typeof call.args !== "object" || Array.isArray(call.args)) return "/protocol";
  const args = call.args as Record<string, unknown>;
  const path = typeof args.path === "string" ? args.path : "/protocol";
  const filePath = typeof args.filePath === "string" ? args.filePath : "/protocol";
  ...
  if (counts.running > 0) {
    const progress = [
      counts.completed > 0
        ...
        : "/protocol",
      counts.failed > 0 ? formatTranslation(t, "agent.read.failedCount", { count: counts.failed }) : "/protocol",
    ].filter(Boolean);
