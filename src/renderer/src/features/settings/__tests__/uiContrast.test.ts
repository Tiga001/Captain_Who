...
describe('uiContrast', () => {
  ...
  it('leaves colors unchanged at 0 and mixes toward ink above that', () => {
    expect(getUiContrastMixPercents(0)).toEqual({ border: 0, text: 0 })
    ...
    expect(mixes.text).toBeGreaterThan(0)
    expect(mixes.border).toBeGreaterThan(mixes.text)
  })

  it('only remaps muted text, secondary icons, and chrome borders in generated CSS variables', () => {
