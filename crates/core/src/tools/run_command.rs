...
impl AgentTool for RunCommandTool {
    ...
    fn definition(&self) -> AgentToolDefinition {
        AgentToolDefinition {
            ...
            description: "Run one bounded non-interactive shell command through the host for builds, tests, queries, dependency management, or program execution. Before the first ordinary call, inspect World State workspace.binding and construct cwd correctly. When a workspace is selected, cwd may be omitted to use the workspace root, or set to a workspace-relative directory; permitted absolute directories and system path aliases also retain their current meaning. When no workspace is selected, the first call must include cwd: cwd is mandatory and must not be omitted even when the command, executable, or arguments already use absolute paths. Set cwd to the existing directory where the command should run, normally the target file's parent directory, using an absolute directory or one of the supported system path aliases @home, @desktop, @documents, or @downloads, optionally followed by a safe child path such as @desktop/project-dir. A no-workspace ordinary call also requires the current write scope to allow all locations. A relative cwd or `.` is invalid without a workspace. The only omission exception is a backend-recognized managed PDF command whose activated PDF Skill supplies a private working directory through its Host-owned contract; follow that Skill's instructions and do not invent a Host path. Never use run_command, shell redirection, a heredoc, or an inline script as an alternate writer for ordinary text/code/config files or to bypass file-write approval; use apply_patch for short Direct changes and write_file for long or staged content. Explicit managed Skill/Builder workflows retain their narrower Host-owned contracts. The command string may contain multiple lines; the Host normalizes line endings and evaluates every newline, pipeline, `&&`, `||`, and `;` segment before execution. A safely quoted heredoc delimiter such as `<<'PY'` is supported for bounded non-writing command input; unquoted delimiters, here-strings, background execution, and shell-interpreter heredocs are denied. Command policy may execute the command automatically, request explicit user approval, or deny catastrophic/unsupported operations. This policy is not an OS sandbox. The Host owns process lifetime and its short initial yield; do not add a deadline merely to bound tool waiting or confirm startup. A command that outlives that initial yield returns status=running with a sessionId; running is not final success. For a GUI app or long-lived server, normally finish the turn after confirming startup instead of waiting for natural exit. For a build, test, or other serial command whose final result is required, call command_session once with action=wait and let the Host wait quietly; do not repeatedly poll or narrate waiting. Background output and exit update Host state but never start a new model turn. For a backend-verified Office Skill Builder materialized in this run, use one direct Python/Node command with `--output <file.docx|file.xlsx|file.pptx>`; the host binds the matching managed runtime and observes the output, so runtimeProfile and observe are not required. Trusted activated built-in Skills may also bind a managed runtime and output publication contract; follow the activated Skill instructions rather than guessing Host paths or runtimeProfile. inputs may bind authorized files read-only for any command. Give each input only the path returned by another tool or supplied by the user; the host recognizes workspace, absolute/system, attachment, browser-download, generated Artifact, and revision-bound Skill paths automatically. The process sees only `$MYCOPILOT_INPUT_ROOT/<mountPath>`, never a private Host storage path.".to_string(),
            input_schema: json!({
                "type": "object",
                ...
                "properties": {
                    ...
                    "inputs": {
                        ...
                        "description": "Optional read-only inputs for any command. Pass one authorized path per file; the host freezes hash/size before approval, revalidates it before execution, and exposes only `$MYCOPILOT_INPUT_ROOT/<mountPath>` to the process. Use this mechanism to copy a browser-download reference into the workspace without exposing its real host path.",
                        "items": {
                            "type": "object",
                            "properties": {
                                "path": {
