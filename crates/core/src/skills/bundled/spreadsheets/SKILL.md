...
- **Read and verify:** use the flat semantic `office_spreadsheet` tool only for `status`, `inspect`, `validate`, and `render`.
- **Create:** use one saved Managed Builder from `templates/builder.py` with normal Python and the pinned `openpyxl`/`xlsxwriter` runtime.
- **Edit an existing `.xlsx`:** inspect first, then use one saved Workbook Editor from `templates/editor.py`. It executes normal Python against the frozen source snapshot and publishes a distinct validated save-as output.
...
Omit `runtimeProfile` and `observe`. Bind every source workbook, image, CSV, attachment, or earlier artifact through `run_command.inputs` and use its logical `mountPath` below `MYCOPILOT_INPUT_ROOT`. The Host freezes the script, runtime, inputs, and target; redirects the declared Office output through private staging; validates it; and publishes atomically. If a command returns `running`, wait on that same command session before using the output. Inspect `artifactObservation` after every terminal result and never blindly repeat a call that may have changed files.

`openpyxl` stores formulas but does not calculate them. Package validation, formula inspection, and cached values do not prove recalculation. Claim calculated results only when a separate authoritative calculation engine returned explicit evidence.
...
4. Inspect the final sheet list, populated ranges, formula text, formats, tables, charts, images, and defined names; then run native validation.
