...
Image, template, data, and deck inputs use the shared `AgentFileInputRef` object rather than
guessed paths. For an attachment, call `attachments_list` and copy its exact `readPath`. For an
earlier generated image, use a `generated_artifact` reference. If the backend returns
...
```json
{
  ...
  "source": {
    "type": "attachment",
    "readPath": "@attachments/<attachment-id>/hero.png"
  },
  "slideNumber": 2,
  "x": "7in",
  "y": "1.6in",
