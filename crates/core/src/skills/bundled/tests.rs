...
fn pdf_skill_is_a_trusted_package_with_progressive_references() {
    ...
    assert!(
        reading.contains("pdftoppm -f 1 -l 12 -png \"<exact outputs[].readPath>\" outputs/qa-page")
    );
...
fn skill_creator_is_a_resource_only_native_workflow_package() {
    ...
    let platform_workflows = package
        ...
        .unwrap();
    let platform_workflows = String::from_utf8(reader.read(platform_workflows).unwrap()).unwrap();
...
fn document_and_spreadsheet_skills_define_second_round_python_script_contracts() {
    let source = BundledSkillSource::new().unwrap();
    for (local_id, workflow_source, directory_command, materialized_destination, template_path) in [
        (
            DOCUMENTS_LOCAL_ID,
            include_str!("documents/references/workflows.md"),
        ...
        (
            SPREADSHEETS_LOCAL_ID,
            include_str!("spreadsheets/references/workflows.md"),
        ...
        let template_uri = package_uri.resource(SkillResourcePath::parse(template_path).unwrap());
        let workspace = tempdir().unwrap();
    ...
    let documents: Value =
        serde_json::from_str(include_str!("documents/office-capability.json")).unwrap();
    assert_eq!(
    ...
    let spreadsheets: Value =
        serde_json::from_str(include_str!("spreadsheets/office-capability.json")).unwrap();
    assert_eq!(
...
fn documents_skill_exposes_one_full_python_existing_document_editor() {
    let workflows = include_str!("documents/references/workflows.md");
    let editing = include_str!("documents/references/editing-existing.md");
    let editor = include_str!("documents/templates/editor.py");
    let capability: Value =
        serde_json::from_str(include_str!("documents/office-capability.json")).unwrap();
...
fn spreadsheets_skill_exposes_one_full_python_existing_workbook_editor() {
    let workflows = include_str!("spreadsheets/references/workflows.md");
    let editing = include_str!("spreadsheets/references/editing-existing.md");
    let editor = include_str!("spreadsheets/templates/editor.py");
    let capability: Value =
        serde_json::from_str(include_str!("spreadsheets/office-capability.json")).unwrap();
...
fn office_skill_native_json_examples_use_flat_semantic_requests() {
    let documents = [
        ...
        (
            ...
            include_str!("presentations/references/workflows.md"),
        ),
    ];
...
fn presentations_skill_requires_separate_builder_syntax_preflight() {
    let workflows = include_str!("presentations/references/workflows.md");
...
fn presentations_skill_exposes_one_fixed_existing_deck_editor_contract() {
    let workflows = include_str!("presentations/references/workflows.md");
    let editing = include_str!("presentations/references/editing-existing.md");
    let editor = include_str!("presentations/templates/editor.mjs");
    let capability: Value =
        serde_json::from_str(include_str!("presentations/office-capability.json")).unwrap();
    ...
    let resources = service.resource_session(&activated).unwrap();
    let package = resources.package_uris().into_iter().next().unwrap();
    let editor_uri = package.resource(SkillResourcePath::parse("templates/editor.mjs").unwrap());
    let materialized_source = resources
        .read_text(&editor_uri, SkillResourceTextReadOptions::default())
        .unwrap();
    assert_eq!(materialized_source.text(), editor);
