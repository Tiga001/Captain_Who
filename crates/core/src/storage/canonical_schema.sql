...
-- Identity lookup belongs in an ordinary indexed table, not FTS UNINDEXED columns.
CREATE TABLE conversation_history_index_entries (
    rowid INTEGER PRIMARY KEY,
    ref_key TEXT NOT NULL UNIQUE,
    archive_ref TEXT,
    owner_message_id TEXT NOT NULL REFERENCES messages(id) ON DELETE CASCADE
);
CREATE INDEX conversation_history_index_entries_archive
    ON conversation_history_index_entries(archive_ref);
CREATE INDEX conversation_history_index_entries_owner
    ON conversation_history_index_entries(owner_message_id);
CREATE TRIGGER conversation_history_index_entries_delete
AFTER DELETE ON conversation_history_index_entries
BEGIN
    DELETE FROM conversation_history_fts WHERE rowid = OLD.rowid;
END;
...
BEGIN
    ...
    SELECT 'trace:' || NEW.assistant_message_id || ':' || NEW.sequence,
        json_extract(NEW.item_json, '$.archiveRef'), NEW.assistant_message_id
    WHERE NEW.item_kind != 'agent_mailbox_delivery';
    INSERT INTO conversation_history_fts (
        rowid,
        ref_key, conversation_id, record_type, item_kind, message_id,
        assistant_message_id, sequence, archive_ref, call_id, tool,
    ...
    SELECT
        (SELECT rowid FROM conversation_history_index_entries
         WHERE ref_key = 'trace:' || NEW.assistant_message_id || ':' || NEW.sequence),
...
BEGIN
    DELETE FROM conversation_history_index_entries
...
BEGIN
    DELETE FROM conversation_history_index_entries WHERE ref_key = 'archive:' || OLD.archive_ref;
END;
