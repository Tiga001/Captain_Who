...
/// multi-call batch crashed before publishing later calls. DeepSeek hydration will independently
/// reject an incomplete projection, while Generic must still observe the provider boundary.
/// Staged rows with no matching durable call remain invisible orphans.
