...
const BROWSER_PDF_SAVE_DESCRIPTION: &str = "Save the current page as a managed PDF artifact using native webpage printing. The published result includes readPath as an artifact://sha256/... URI; pass that exact value to run_command.inputs[].path to use the PDF in a later command. Do not guess a filesystem path from filename, displayName, or artifactId, or pass the URI directly to shell commands. Pages with cross-process embedded frames are currently unsupported. A typed unavailable result explains compatibility limits or a pending print; follow its recovery guidance before retrying.";
