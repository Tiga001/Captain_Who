        ...
        if !record.snapshot.status.is_terminal() {
            ...
            let control = if let Some(terminal) = live.terminal() {
                ...
                match request.action {
                    AgentCommandSessionAction::Interrupt => self
                        .inner
                        .manager
                        .interrupt(&id, deadline.saturating_duration_since(Instant::now()))
                        .map(|snapshot| {
                            (
                                snapshot.latest_output_sequence,
                    ...
                    AgentCommandSessionAction::Poll => {
                        ...
                        self.inner
                            ...
                            .map(|poll| {
                                (
                                    poll.snapshot.latest_output_sequence,
                                    poll.snapshot.state.is_terminal(),
                                )
                            })
                    }
                }
            ...
            match control {
                Ok((sequence, terminal_observed)) => {
                    ...
                    if terminal_observed && live.terminal().is_none() {
                        live.wait_for_terminal(deadline.saturating_duration_since(Instant::now()));
                    }
                }
                Err(error) => {
            ...
            live.wait_for_persisted_sequence(
                observed_sequence,
                deadline.saturating_duration_since(Instant::now()),
            );
            if live.terminal().is_some() {
                ...
                live.wait_for_terminal_settled(deadline.saturating_duration_since(Instant::now()));
            }
        }
