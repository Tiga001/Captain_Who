...
mod tests {
    ...
    #[test]
    fn confirmed_reset_failure_preserves_current_v11_source_and_recovery_backup() {
        let fixture = tempfile::tempdir().unwrap();
        ...
        // A canonical v11 database uses WAL mode. Exercise the actual recovery shape by restoring
        // the immutable snapshot to a writable database identity instead of mutating the only
        // backup merely to inspect it.
        let recovered_database = fixture.path().join("recovered-v11.sqlite");
