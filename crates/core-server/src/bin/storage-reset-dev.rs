...
const RECOVERABLE_CONFIGURATION_TARGET_SCHEMA_VERSION: i32 = 39;
...
mod tests {
    ...
    #[test]
    fn explicit_configuration_sources_are_pinned_to_known_catalogs_for_schema_39() {
    ...
    fn exact_v35_reset_preserves_configuration_and_credentials_without_migrating_history() {
        ...
        drop(storage);
        let connection = open_read_only(&database).unwrap();
        assert_eq!(storage_schema_version(&connection).unwrap(), 39);
        assert_eq!(
            snapshot_exact_configuration_tables(&connection).unwrap(),
            exact_before
        );
