...
| 文件修改      | 通过统一 FileChange 结构化创建、更新或删除文件   |
...
`apply_patch` 是当前唯一的结构化文件修改 Tool。小修改使用一次性 Direct FileChange，大内容可在同一个 Staged FileChange 中分段组装；两者使用相同的审批、冲突检测和结果状态。

更新或删除文件前，Agent 必须先读取并绑定准确版本。审批卡会加载完整或分页 Diff；批准后仍会重新核对文件、父目录和路径边界。成功后，时间线保留可再次打开的审阅卡；文件内容不会作为完整副本长期塞进普通运行轨迹。
