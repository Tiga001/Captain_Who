...
- 超大文件修改会受到受管草稿和补丁大小限制；应让 Agent 分阶段写入或缩小单次修改范围。
...
- Skill Script 当前只支持 `scripts/*.py` 和宿主 Python 3；缺失依赖不会自动安装。预检和执行需要高权限条件，真正运行每次都需审批。
...
- 浏览器 Artifact 是临时 Run 产物，不是永久文件；最长生命周期为 24 小时。需要长期保留时应显式导出。
...
- 同一 Agent 同时最多一个活跃 Turn。模板按项目隔离，修改只影响未来节点。
