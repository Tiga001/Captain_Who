...
- 仅直接面向用户的根 Agent 可用；所有层级子 Agent 与无人值守定时运行都不挂载。
- 每批没有产品层面的题数上限，但受通用请求大小、问题文字、选项与回答文字长度约束。
- 不支持答题附件、多人共同编辑同一批草稿或提交后重新编辑。每批只接纳一次正式提交或忽略。
- 未答问题与已提交答案可随同版本重启恢复；未提交草稿只保存在当前窗口内存。
- 结果未知的恢复不自动重放模型或工具请求，需要核对实际状态后明确继续。
