...
Captain Who 采用本地优先的桌面架构：应用状态主要保存在本机，Agent 的 Tool 调用经过类型校验、权限判断和必要的用户审批。账户登录和软件许可校验是独立的账户服务流程，不会同步桌面项目或对话。应用不是通用操作系统沙箱，不能替代账户隔离、磁盘加密、备份和终端安全软件。
...
- 正式签名 macOS 发行版在完整初始化后，通过可访问的官方更新源发现版本；由用户选择开始下载，下载、完整性校验和原生准备完成后，应用会退出并重新启动以安装。更新请求不携带账户授权或 Cookie 作为更新身份标识。
...
- 账户、会话和许可校验需要网络及可用的账户服务；无法完成校验时，新的 Agent 回合可能无法启动。该校验不代表账户服务可读取桌面任务内容。
