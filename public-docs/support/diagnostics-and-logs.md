...
应用数据目录包含数据库、附件、已安装 Skill、生成文件及其他受管数据；未签名 macOS 开发构建的私有凭据文件也位于其中。当前版本的 `storage.sqlite` 只保存凭据引用与状态，不含当前模型/搜索 secret，但旧版本数据库或备份仍可能含明文；数据库也包含私密业务内容，绝不是普通日志，不应上传到公开问题。已保存的账户会话同样不应作为诊断材料分享。
...
不要把可利用细节、真实 Token、他人数据或完整应用数据目录发布到公开讨论中。怀疑凭据暴露时，应立即到相应第三方服务撤销或轮换凭据。需要联系项目方时，按[自助排查](README.md)中的邮箱与脱敏要求操作。
