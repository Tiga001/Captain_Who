...
应用数据目录包含数据库、附件、已安装 Skill、生成文件及其他受管数据；未签名 macOS 开发构建的私有凭据文件也位于其中。当前版本的 `storage.sqlite` 只保存凭据引用与状态，不含当前模型/搜索 secret，但旧版本数据库或备份仍可能含明文；数据库也包含私密业务内容，绝不是普通日志，不应上传到公开问题。
